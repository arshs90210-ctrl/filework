...
function showTooltip(e, txt) { const tt = document.getElementById('tooltip'); tt.style.display = 'block'; tt.style.left = e.pageX + 10 + 'px'; tt.style.top = e.pageY + 10 + 'px'; tt.innerText = txt; }
...
function renderSequencingTable() { const tb = document.getElementById('sequencingTableBody'); tb.innerHTML = Object.keys(ROUTES).sort().map(f => { const steps = ROUTES[f]; const routeStr = steps.map(s => `[${s.pool.join('/')}]`).join(' <i class="fa-solid fa-arrow-right text-xs mx-1 text-slate-300"></i> '); return `<tr class="hover:bg-slate-50 border-b border-slate-100"><td class="p-4 font-bold text-blue-600">${f}</td><td class="p-4 text-xs font-mono">${routeStr}</td><td class="p-4 text-center"><button onclick="openRouteEdit('${f}')" class="text-blue-500 hover:underline text-xs admin-only"><i class="fa-solid fa-pen"></i></button></td></tr>`; }).join(''); }
function openRouteEdit(felt) { if(USER_ROLE !== 'admin') return alert('Admin only'); const steps = ROUTES[felt] || []; const str = steps.map(s => s.pool.join('/')).join(', '); document.getElementById('editRouteFelt').innerText = felt; document.getElementById('editRouteString').value = str; document.getElementById('editRouteModal').style.display = 'flex'; }
function saveRouteEdit() { const felt = document.getElementById('editRouteFelt').innerText; const raw = document.getElementById('editRouteString').value; const parts = raw.split(','); const newSteps = parts.map((p, i) => { const machines = p.split('/').map(m => m.trim()); return { step: i+1, pool: machines }; }); ROUTES[felt] = newSteps; document.getElementById('editRouteModal').style.display = 'none'; renderSequencingTable(); saveLocalState(); runEngine(); }
function applyMassRoute() { if(USER_ROLE !== 'admin') return alert('Admin only'); const filter = document.getElementById('massRouteFilter').value.toUpperCase(); const raw = document.getElementById('massRouteString').value; if(!filter || !raw) return alert("Please fill both fields"); const parts = raw.split(','); const newSteps = parts.map((p, i) => { const machines = p.split('/').map(m => m.trim()); return { step: i+1, pool: machines }; }); let count = 0; Object.keys(ROUTES).forEach(k => { if(k.toUpperCase().includes(filter)) { ROUTES[k] = newSteps; count++; } }); RAW_ORDERS.forEach(o => { if(o.felt.toUpperCase().includes(filter) && !ROUTES[o.felt]) { ROUTES[o.felt] = newSteps; count++; } }); alert(`Updated routes for ${count} Felt Codes.`); document.getElementById('massRouteModal').style.display='none'; renderSequencingTable(); saveLocalState(); runEngine(); }
function renderRatesTable() { const tb = document.getElementById('ratesTableBody'); tb.innerHTML = Object.keys(RATES).sort().map(k => `<tr class="hover:bg-slate-50 transition"><td class="p-4 font-mono text-blue-600 font-bold">${k}</td><td class="p-4 text-right">${RATES[k].card}</td><td class="p-4 text-right">${RATES[k].finish}</td><td class="p-4 text-right">${RATES[k].qc}</td><td class="p-4 text-center"><button class="text-blue-500 hover:text-blue-700 mr-2 admin-only" onclick="openRateEdit('${k}')"><i class="fa-solid fa-pen"></i></button><button class="text-red-400 hover:text-red-600 admin-only" onclick="deleteRate('${k}')"><i class="fa-solid fa-trash"></i></button></td></tr>`).join(''); }
function openRateEdit(felt) { if(USER_ROLE !== 'admin') return alert('Admin only'); const r = RATES[felt]; document.getElementById('editRateFelt').innerText = felt; document.getElementById('editRateCard').value = r.card; document.getElementById('editRateFin').value = r.finish; document.getElementById('editRateQC').value = r.qc; document.getElementById('editRateModal').style.display = 'flex'; }
function saveRateEdit() { const felt = document.getElementById('editRateFelt').innerText; const c = parseFloat(document.getElementById('editRateCard').value); const f = parseFloat(document.getElementById('editRateFin').value); const q = parseFloat(document.getElementById('editRateQC').value); RATES[felt] = { card:c, finish:f, qc:q }; document.getElementById('editRateModal').style.display = 'none'; renderRatesTable(); runEngine(); }
function addNewRateRow() { if(USER_ROLE !== 'admin') return alert('Admin only'); const f = prompt("Enter Felt Code:"); if(f) { RATES[f] = { card:250, finish:700, qc:400 }; renderRatesTable(); } }
function renderStaffingTable() { const tb = document.getElementById('staffingTableBody'); tb.innerHTML = ''; CONFIG.machines.forEach(m => { const s = STAFFING[m]; tb.innerHTML += `<tr class="border-b border-slate-100 hover:bg-slate-50"><td class="p-4 font-bold text-slate-700">${m}</td><td class="p-4 text-center"><input type="checkbox" ${s[0]?'checked':''} onchange="updateStaffing('${m}', 0, this.checked)" class="w-5 h-5 text-blue-600 rounded"></td><td class="p-4 text-center"><input type="checkbox" ${s[1]?'checked':''} onchange="updateStaffing('${m}', 1, this.checked)" class="w-5 h-5 text-blue-600 rounded"></td><td class="p-4 text-center"><input type="checkbox" ${s[2]?'checked':''} onchange="updateStaffing('${m}', 2, this.checked)" class="w-5 h-5 text-blue-600 rounded"></td></tr>`; }); }
